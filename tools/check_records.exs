# tools/check_records.exs — the record checks: authorship, exchange
# schema, deps. This is what the pre-commit hook and the PR-required CI
# run; view reproducibility (check_views) is main's concern and lives in
# check_all / the derivation job (docs/workflow.md).
#
# #agent-authored — provenance: exchanges/2026-08-15-bootstrap-thread.md

defmodule CheckRecords do
  @checks [
    "tools/check_authorship.exs",
    "tools/check_exchanges.exs",
    "tools/check_deps.exs"
  ]

  def main do
    results =
      Enum.map(@checks, fn script ->
        {out, code} = System.cmd("elixir", [script], stderr_to_stdout: true)
        if String.trim(out) != "", do: IO.write(out)
        {script, code}
      end)

    if Enum.all?(results, fn {_, code} -> code == 0 end) do
      System.halt(0)
    else
      failed = for {s, c} <- results, c != 0, do: s
      IO.puts("check_records: FAILED: #{Enum.join(failed, ", ")}")
      System.halt(1)
    end
  end
end

CheckRecords.main()
